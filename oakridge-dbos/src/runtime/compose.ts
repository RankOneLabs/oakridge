...
export const createOakridgeRuntime = async (config: OakridgeRuntimeConfig): Promise<OakridgeRuntime> => {
  ...
  const app = createApp({
    ...
    gate_resume: { contexts, artifacts, collaboration, audits, get_gate_state: getGateState, send_gate_command: sendGateWorkflowCommand,
      get_handoff_state: getHandoffState, send_handoff_command: sendHandoffWorkflowCommand, records: runRecords, now },
    handoff_complete: { artifacts, contexts, get_handoff_state: getHandoffState, send_handoff_command: sendHandoffWorkflowCommand, records: runRecords, now },
