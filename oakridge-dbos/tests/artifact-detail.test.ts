...
const revision = (id: string, version: number): ArtifactRevision => ({ id: id as ArtifactId, chain_id: "artifact-1" as ArtifactId, run_id: "run-1" as WorkflowRunId, stage_instance_id: "stage-1" as StageInstanceId, execution_id: "execution-1" as ExecutionId, unit_id: "unit-1" as UnitId, output_name: "result", artifact_type: "dev.result", label: "web", body: { version }, version, parent_artifact_id: version === 1 ? null : `artifact-${version - 1}` as ArtifactId, created_at: `2026-08-14T0${version}:00:00Z` });
...
test("artifact detail derives revision status from applied gate decisions", async () => {
  ...
  const app = createArtifactDetailApp({
    artifacts: { emit_revision: async () => second, find_by_id: async () => second, find_tip: async () => second, list_chain: async () => [first, second] },
  ...
  expect(await response.json()).toEqual(expect.objectContaining({ id: "artifact-2", producing_stage: "build", revisions: [expect.objectContaining({ id: "artifact-1", status: "draft" }), expect.objectContaining({ id: "artifact-2", status: "approved" })] }));
});
