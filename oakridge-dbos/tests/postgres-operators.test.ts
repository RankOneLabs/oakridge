...
test("run detail derives stages and units from DBOS child workflows", async () => {
  ...
  const executor: SqlExecutor = { query: async <Row>(sql: string) => {
    ...
    if (sql.includes("stage-admission-state") && sql.includes("stage.run_id = $1")) return [{ stage_instance_id: "stage-1", unit_id: "web", params: { title: "Web" }, external_reference: { kind: "kbbl_session", session_id: "session-1", worktree_base_sha: "a".repeat(40) }, dbos_status: "PENDING", has_pending_gate: true, admission_required: true, admitted: false, admission_eligible: true, admission_blocked_by: [] }] as Row[];
  ...
  expect(detail).toEqual(expect.objectContaining({ current_attempt_root_workflow_id: "root-2", status: "parked",
    ...
    epic_profile: expect.objectContaining({ id: "epic-1", lifecycle_state: "active" }), stages: [expect.objectContaining({ name: "build", status: "parked", delegated_kbbl_sid: "session-1", units: [expect.objectContaining({ unit_id: "web", base_sha: "a".repeat(40), status: "parked", admission_required: true, admitted: false, admission_eligible: true })] })] }));
});
