...
class TestConditionResolution:
    ...
    def test_single_agent_n_is_always_1(self) -> None:
        spec = RunSpec.from_dict(
            _spec(condition={"kind": "single_agent", "n": 1})
        )
        condition = registry.CONDITION_FACTORIES[spec.condition.kind](n=spec.condition.n)
...
class TestNBounds:
    def test_n_zero_raises(self) -> None:
        with pytest.raises(ValueError):
            RunSpec.from_dict(
                _spec(condition={"kind": "ensemble_incremental", "n": 0})
            )
    ...
    def test_n_17_raises(self) -> None:
        with pytest.raises(ValueError):
            RunSpec.from_dict(
                _spec(condition={"kind": "ensemble_incremental", "n": 17})
            )

    def test_n_16_valid(self) -> None:
        spec = RunSpec.from_dict(
            _spec(condition={"kind": "ensemble_incremental", "n": 16})
        )
        assert spec.condition.n == 16

    def test_n_1_valid(self) -> None:
        spec = RunSpec.from_dict(
            _spec(condition={"kind": "single_agent", "n": 1})
        )
...
class TestKindSpecificN:
    def test_single_agent_n2_raises(self) -> None:
        with pytest.raises(ValueError, match="single_agent"):
            RunSpec.from_dict(
                _spec(condition={"kind": "single_agent", "n": 2})
            )

    def test_ensemble_single_round_n1_raises(self) -> None:
        with pytest.raises(ValueError):
            RunSpec.from_dict(
                _spec(condition={"kind": "ensemble_single_round", "n": 1})
            )

    def test_ensemble_multi_round_n1_raises(self) -> None:
        with pytest.raises(ValueError):
            RunSpec.from_dict(
                _spec(condition={"kind": "ensemble_multi_round", "n": 1})
            )

    def test_ensemble_incremental_n1_valid(self) -> None:
        spec = RunSpec.from_dict(
            _spec(condition={"kind": "ensemble_incremental", "n": 1})
        )
...
class TestModelPoolValidation:
    def test_empty_model_pool_raises(self) -> None:
    ...
    def test_empty_string_only_raises(self) -> None:
        with pytest.raises(ValueError):
            RunSpec.from_dict(_spec(model_pool=[""]))

    def test_non_list_model_pool_raises(self) -> None:
        with pytest.raises(ValueError, match="model_pool"):
...
class TestUnknownKeys:
    ...
    def test_unknown_kind_raises(self) -> None:
        with pytest.raises(ValueError, match="condition.kind"):
            RunSpec.from_dict(
                _spec(condition={"kind": "unknown_kind", "n": 1})
            )
...
class TestGradeField:
    ...
    def test_grade_false_accepted(self) -> None:
        spec = RunSpec.from_dict(_spec(grade=False))
        assert spec.grade is False

    def test_grade_non_bool_raises(self) -> None:
...
class TestEventTee:
    async def test_tee_writes_expected_shape(self, tmp_path: Path) -> None:
    ...
    async def test_tee_creates_parent_dir(self, tmp_path: Path) -> None:
        jsonl_path = tmp_path / "deep" / "nested" / "events.jsonl"
        tee = _build_event_tee(jsonl_path)
        await tee("ping", {})
        assert jsonl_path.exists()

    async def test_tee_appends_multiple_lines(self, tmp_path: Path) -> None:
