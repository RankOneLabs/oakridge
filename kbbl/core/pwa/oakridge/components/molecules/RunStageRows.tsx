...
export function RunUnitRow({ stageName, stageType, unit, unitArtifacts, onSelectArtifact, onAdmit, admitting, admissionError, onRetry, retrying, retryError }: RunUnitRowProps) {
  ...
  return (
    ...
    <tr className={stageRowClass(unit.status)} data-testid="or-stage-row">
      ...
      <td className={tableCellClass}><div className="flex items-center gap-2">
        ...
        {unit.status === "failed" && <button type="button" className="rounded border border-red-500 px-2 py-0.5 text-xs text-red-500 hover:bg-red-500 hover:text-white disabled:opacity-50" onClick={() => onRetry(unit.unit_id)} disabled={retrying} data-testid="or-retry-unit-btn">{retrying ? "Retrying…" : "Retry"}</button>}
