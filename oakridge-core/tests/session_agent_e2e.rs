...
"#,
    ...
    // Keep a handle to live_stages so the test can peek at pending approval request IDs
    let live_stages: Arc<Mutex<HashMap<StageInstanceId, LiveStage>>> =
        Arc::new(Mutex::new(HashMap::new()));
    let live_stages_test = live_stages.clone();
    ...
    // Retrieve the pending approval request_id generated by hook_approval_handler
    let request_id: String = {
        let deadline = tokio::time::Instant::now() + Duration::from_secs(5);
        loop {
            {
                let map = live_stages_test.lock().unwrap();
                if let Some(ls) = map.get(&si_id) {
                    if let Some(req_id) = ls.pending_approvals.keys().next() {
                        break req_id.clone();
                    }
                }
            }
            assert!(
                tokio::time::Instant::now() < deadline,
                "pending approval request_id did not appear within timeout"
            );
            tokio::time::sleep(Duration::from_millis(50)).await;
        }
    };
