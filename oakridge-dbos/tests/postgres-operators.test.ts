...
test("run detail derives stages and units from DBOS child workflows", async () => {
  const executor: SqlExecutor = { query: async <Row>(sql: string) => {
    ...
    if (sql.includes("FROM oakridge.executor_projection")) return [{ stage_instance_id: "stage-1", unit_id: "web", params: { title: "Web" }, external_reference: { kind: "kbbl_session", session_id: "session-1" }, dbos_status: "PENDING", has_pending_gate: true }] as Row[];
  ...
  expect(detail).toEqual(expect.objectContaining({ current_attempt_root_workflow_id: "root-2", status: "parked",
    ...
    epic_profile: expect.objectContaining({ id: "epic-1", lifecycle_state: "active" }), stages: [expect.objectContaining({ name: "build", status: "parked", delegated_kbbl_sid: "session-1", units: [expect.objectContaining({ unit_id: "web", status: "parked" })] })] }));
});

test("review inbox maps pending DBOS gates to existing operator actions", async () => {
  const executor: SqlExecutor = { query: async <Row>(sql: string) => {
    if (sql.includes("handoff-state")) return [{ run_id: "run-1", workflow_name: "dev-flow", stage_instance_id: "stage-1", stage_name: "build", unit_id: "web", params: { repository_key: "web", title: "Web" }, dbos_status: "PENDING", artifact_revision_id: "artifact-1", handoff_status: "awaiting_downstream", updated_at_epoch_ms: "1786737600000" }] as Row[];
  ...
  expect(inbox.cohorts).toEqual([expect.objectContaining({ id: "stage-1:web", lifecycle: "artifact_review", repository_key: "web", gate_id: "stage-1:web", artifact_url: "/artifact_details/artifact-1", completion: { build_complete: true, assessment_complete: false } })]);
});
...
test("cohort projection surfaces validated pull request mismatches as blocked inbox state", async () => {
  ...
  const executor: SqlExecutor = { query: async <Row>(sql: string) => {
    if (sql.includes("handoff-state")) return [{ run_id: "run-1", workflow_name: "dev-flow", stage_instance_id: "stage-1", stage_name: "build", unit_id: "web", params: { repository_key: "web" }, dbos_status: "PENDING", artifact_revision_id: "artifact-1", handoff_status: "awaiting_external", reconciliation, updated_at_epoch_ms: "1786737600000" }] as Row[];
