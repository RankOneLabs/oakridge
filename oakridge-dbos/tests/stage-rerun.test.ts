...
test("whole-stage rerun starts a new DBOS attempt from artifact ancestry without an app-owned continuation plan", async () => {
  ...
  const dependencies = {
    ...
    attempts: { async list_for_run() { return [{ root_workflow_id: "root-1", run_id: runId, forked_from_root_workflow_id: null, created_at: "2026-08-14T00:00:00Z" }]; },
      async find_by_root_workflow_id() { return null; }, async insert(attempt: WorkflowAttempt) { insertedRoot = attempt.root_workflow_id; } },
    dbos: { async start_run(workflow_id: string, input: RunWorkflowInput) { started = { workflow_id, resume_from_stage: input.resume_from_stage }; } },
    now: () => "2026-08-14T01:00:00Z",
    supersede_attempt: async (root: string) => { supersededRoot = root; },
