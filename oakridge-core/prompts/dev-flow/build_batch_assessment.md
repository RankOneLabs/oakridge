...
Empty arrays are valid for `changed_files` and `known_issues`. If all tests pass, `failed` should be 0. After `build_result` has been successfully posted, stop. The batch assessor starts only after every build cohort has finished.
