...
import type { RunRecordVersion, WaitId } from "../src/domain/primitives";
...
test("a gated output reports its pending wait rather than a release", async () => {
  ...
  const app = createWorkOrderArtifactCallbackApp({ records: { publish_artifact: async (request: PublishWorkOrderArtifact) =>
    ({ kind: "pending", artifact_id: request.artifact_id, wait_id: waitId, record_version: 5 as RunRecordVersion }) }, now: () => "2026-08-28T12:00:00.000Z" });
