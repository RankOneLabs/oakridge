...
To run planner-2 only without creating a build phase (legacy dry-run, sets run
status to `completed` rather than `awaiting_review`):
...
already requires an approved brief. `--from-brief` and `--dry-run` are also
mutually exclusive — the build-only pipeline has no planner-2 step to skip.
