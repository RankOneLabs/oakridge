...
test("production services load and compile the immutable requested definition version", async () => {
  ...
  const services = createProductionTopologyServices({ definitions, runs: {} as WorkflowRunRepository, attempts: {} as WorkflowAttemptRepository,
    ...
    resume_artifacts: { async list_latest_for_stages() { return []; }, async list_released_for_execution() { return []; } }, waits: { async find_handoff_waits_for_artifact() { return []; } }, load_prompt_template: async () => "template" });
...
test("production stage service reads a revision's handoff standing from its wait rows", async () => {
  ...
  const services = createProductionTopologyServices({ definitions: {} as WorkflowDefinitionRepository, runs: {} as WorkflowRunRepository,
    ...
    resume_artifacts: { async list_latest_for_stages() { return []; }, async list_released_for_execution() { return []; } },
...
test("production stage service settles a unit from what it released and what was observed of it", async () => {
  const released = { id: "artifact-1", execution_id: "execution-1", unit_id: "web", output_name: "build_result", lifecycle: { kind: "released", released_at: "2026-08-28T18:50:02Z" } } as never;
  const observed = { kind: "failed", code: "executor_silent_timeout", detail: "quiet for 1826s" } as const;
  const services = createProductionTopologyServices({ definitions: {} as WorkflowDefinitionRepository, runs: {} as WorkflowRunRepository,
    attempts: {} as WorkflowAttemptRepository, stages: {} as StageInstanceRepository,
    executions: { async find_terminal_observation(execution: string) { return execution === "execution-1" ? observed : null; } } as unknown as ExecutionProjectionRepository,
    rerun_targets: { async find_unit_target() { return null; }, async replace_execution_workflow() {} },
    resume_artifacts: { async list_latest_for_stages() { return []; }, async list_released_for_execution(execution) { return execution === "execution-1" ? [released] : []; } },
    waits: { async find_handoff_waits_for_artifact() { return []; } }, load_prompt_template: async () => "template" });
  expect(await services.find_unit_record("execution-1" as never)).toEqual({ released_artifacts: [released], terminal_observation: observed });
  expect(await services.find_unit_record("execution-9" as never)).toEqual({ released_artifacts: [], terminal_observation: null });
});
