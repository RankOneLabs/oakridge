...
mod tests {
    ...
    async fn seeding_retires_every_superseded_dev_flow_version() {
        ...
        assert_eq!(all.len(), 8, "retired defs are kept, not deleted");
    }
    ...
    async fn new_builtin_version_supersedes_an_already_seeded_v8_definition() {
        let path = format!("/tmp/oakridge_seed_upgrade_test_{}.db", uuid::Uuid::new_v4());
        let pool = crate::db::init_pool(&format!("sqlite:{}", path))
            .await
            .unwrap();
        ...
        assert_eq!(
            config.fan_out.unwrap().session_mode,
            crate::executor::delegated_session::config::FanOutSessionMode::Shared
        );
    }
}
