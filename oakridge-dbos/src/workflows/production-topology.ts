...
export const productionStageWorkflow = DBOS.registerWorkflow(async (input: StageWorkflowInput): Promise<StageWorkflowResult> => {
  ...
  const admitted = new Set<UnitId>(input.compiled_stage.materialization.kind === "fan_out" && input.compiled_stage.materialization.manual_admission ? [] : plans.map((plan) => plan.unit.unit_id));
  ...
  while (!incremental.is_closed || released.size < plans.length) {
    ...
    if (ready.length === 0) {
      ...
      if (!command) continue;
      if (command.kind === "admit_unit") { admitted.add(command.unit_id); continue; }
