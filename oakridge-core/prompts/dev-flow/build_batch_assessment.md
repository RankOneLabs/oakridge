...
Empty arrays are valid for `changed_files` and `known_issues`. If all tests pass, `failed` should be 0. After the `build_result` PUT succeeds, stop. The batch assessor starts only after every build cohort has finished.
