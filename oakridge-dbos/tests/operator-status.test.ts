...
test("v2 operator status is selected only from persisted run-owned state", () => {
  ...
  expect(selectV2UnitStatus("working", false, true, true)).toBe("failed");
  expect(selectV2UnitStatus("ready", false, true, true, true)).toBe("pending");
  expect(selectV2UnitStatus("working", false, true, false)).toBe("running");
});
