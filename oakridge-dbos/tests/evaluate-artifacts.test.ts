...
test("successful executor termination does not bypass a pending artifact gate", () => {
  const contract = evaluateExecutionArtifactContract([output({ kind: "gate", steps: [{ type: "artifact_approval", actions: ["approve"] }], requires_zero_open_review_items: false, revision_target: "self_stage" })], []);
